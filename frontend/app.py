    ...
    for field_name, props in schema_fields.items():
        ...
        if field_type == 'list[model]':
            ...
            for i, item_data in enumerate(items):
                ...
                with st.expander(f"{item_label} Item {i+1}", expanded=True):
            ...
            for i, item_text in enumerate(items):
                item_label = props.get('label', field_name.replace('_', ' ').title())
                new_text = st.text_input(f"{item_label} Item {i+1}", value=item_text, key=f"{widget_key}_{i}")
            ...
            if 'choices' in validation:
                data[field_name] = st.selectbox(label, options=validation['choices'], index=validation['choices'].index(default) if default in validation['choices'] else 0, key=widget_key)
            elif field_type == 'bool':
                data[field_name] = st.checkbox(label, value=default if default is not None else False, key=widget_key)
            elif field_type == 'int':
                data[field_name] = st.number_input(label, value=default, step=1, key=widget_key)
            else:
                data[field_name] = st.text_input(label, value=default if default is not None else "", key=widget_key)
    return data
...
def manage_dynamic_lists(schema_fields, is_advanced, parent_key=""):
    ...
    for field_name, props in schema_fields.items():
        ...
        if field_type == 'list[model]':
            ...
            items_in_state = st.session_state.get(widget_key, [])
            for i, item_data in enumerate(items_in_state):
                if st.button(f"Remove {label} Item {i+1}", key=f"{widget_key}_remove_{i}"):
                    st.session_state[widget_key].pop(i)
                    st.rerun()
